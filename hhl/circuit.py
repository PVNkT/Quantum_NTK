...
from hhl.QPE import QPE, qpe_qiskit
from hhl.rotation import rotation, Reciprocal, my_rotation
from hhl.initialize import make_b_state
def circuit(A, b, nl, evolution_time, delta, neg_vals, wrap = True, measurement = None):
    ...
    #회로 그림을 저장
    if wrap:
        qc.draw("mpl").savefig("/workspace/hhl/image/HHL_circuit_wrapped.png")
    else:
        qc.draw("mpl").savefig("/workspace/hhl/image/HHL_circuit_unwrapped.png")
    #회로를 반환
    return qc